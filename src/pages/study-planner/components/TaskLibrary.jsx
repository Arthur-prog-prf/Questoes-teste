...
const TaskLibrary = ({ 
  ...
  // Extract subjects and topics from existing data
  const availableSubjects = existingSubjects?.length > 0 
    ...
    : [
        ...
        { key: 'atualidades', name: 'Atualidades' }
      ];
  ...
  const availableTopics = existingSubjects?.flatMap(subject => 
  ...
  const getTotalUnscheduledHours = () => {
    ...
    return unscheduledTasks?.reduce((total, task) => total + (task?.duration || 1), 0);
  };
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm opacity-90 mb-2">{task?.topic}</p>
                      ...
                      <div className="flex items-center space-x-4 text-xs opacity-75">
                        ...
                        {task?.estimatedSessions && (
                          <div className="flex items-center space-x-1">
                            ...
                            <span>{task?.estimatedSessions} sessões</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-lg shadow-lg border border-border w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-muted rounded-lg">
                <h4 className="font-medium text-foreground">{selectedTaskForSchedule?.topic}</h4>
                <p className="text-sm text-text-secondary mt-1">
                  {availableSubjects?.find(s => s?.key === selectedTaskForSchedule?.subject)?.name || selectedTaskForSchedule?.subject}
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-end space-x-3 p-4 border-t border-border">
              ...
              <Button
                ...
                disabled={!newTaskForm?.subject?.trim()}
                iconName="Plus"
                iconPosition="left"
              >
                 Adicionar Tarefa
